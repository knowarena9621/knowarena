...
import { getNoticesForClass, getDailyQuote } from "./firebase/notices";
...
function StudentHeader({ student, unreadCount=0, onBellClick }) {
  return (
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,position:"relative"}}>
        {onBellClick && (
          <button onClick={onBellClick} style={{position:"relative",background:"rgba(255,255,255,0.15)",border:"none",borderRadius:"50%",width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:17}}>
            🔔
            {unreadCount>0 && (
              <span style={{position:"absolute",top:-3,right:-3,background:T.error,color:"#fff",borderRadius:"50%",minWidth:16,height:16,fontSize:9,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px",border:"1.5px solid #15347a"}}>
...
// Slide-down panel showing teacher notices + today's motivational quote.
function NoticesPanel({ notices, onClose }) {
  const fmtDate = (ts) => {
    if (!ts?.toDate) return "";
    return ts.toDate().toLocaleDateString("en-IN", { day:"2-digit", month:"short", hour:"2-digit", minute:"2-digit" });
  };
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:50,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"70px 16px 0"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:480,maxHeight:"70vh",overflowY:"auto",boxShadow:"0 12px 40px rgba(0,0,0,0.25)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 18px",borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,background:"#fff",borderRadius:"16px 16px 0 0"}}>
          <h3 style={{margin:0,fontSize:16,fontWeight:800,color:T.text}}>📣 Notices</h3>
          <button onClick={onClose} style={{background:T.bg,border:"none",borderRadius:8,width:28,height:28,cursor:"pointer",fontSize:14,color:T.textM}}>✕</button>
        </div>
        <div style={{padding:"12px 16px 18px"}}>
          {notices.map(n=>(
            <div key={n.id} style={{
              background:n.isQuote?T.goldL:T.blueL, borderRadius:12, padding:"12px 14px", marginBottom:10,
              borderLeft:`4px solid ${n.isQuote?T.gold:T.blue}`,
            }}>
              <p style={{margin:"0 0 6px",fontSize:13,color:T.text,fontWeight:n.isQuote?700:500}}>{n.isQuote?"🌟 Quote of the Day: ":""}{n.message}</p>
              {!n.isQuote && <div style={{fontSize:11,color:T.textL}}>{n.teacherName||"Teacher"} · {fmtDate(n.createdAt)}</div>}
            </div>
          ))}
        </div>
      </div>
...
export default function StudentApp({ student, onLogout, showToast }) {
  ...
  // ── Load tests + notices on mount ──────────────────────────────────────────
  useEffect(() => { loadTests(); loadMyResults(); loadNotices(); }, []);

  const loadNotices = async () => {
    try {
      const teacherNotices = await getNoticesForClass(student.cls);
      const dailyQuote = { id: "daily-quote", message: getDailyQuote(), isQuote: true, createdAt: null };
      setNotices([dailyQuote, ...teacherNotices]);
    } catch(e) { console.error(e); }
  };

  const unreadCount = (() => {
    try {
      const lastSeen = Number(localStorage.getItem("ka_notices_last_seen") || 0);
      return notices.filter(n => !n.isQuote && n.createdAt?.toMillis && n.createdAt.toMillis() > lastSeen).length;
    } catch { return 0; }
  })();

  const openNotices = () => {
    setShowNotices(true);
    try { localStorage.setItem("ka_notices_last_seen", String(Date.now())); } catch {}
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",paddingBottom:70}}>
      <StudentHeader student={student} unreadCount={unreadCount} onBellClick={openNotices}/>
      {showNotices && <NoticesPanel notices={notices} onClose={()=>setShowNotices(false)}/>}
...
function DashboardView({ student, tests, myResults, testsLoading, onOpenTest, onGoToTests, onGoToResults, attemptedIds }) {
  ...
  return (
    <div>
      ...
      {/* Login streak — real data, tracked on every login/session restore */}
      {(student.streakCount||0)>0 && (
        <Card style={{marginBottom:18,padding:"14px 16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:800,fontSize:14,color:T.text}}>{student.streakCount} Day Streak 🔥</div>
              <div style={{fontSize:11,color:T.textM,marginBottom:6}}>Keep the streak alive!</div>
              <div style={{display:"flex",gap:5}}>
                {Array.from({length:7}).map((_,i)=>(
                  <div key={i} style={{
                    width:20,height:20,borderRadius:"50%",fontSize:9,fontWeight:800,
                    display:"flex",alignItems:"center",justifyContent:"center",
                    background:i<Math.min(student.streakCount,7)?T.success:T.bg,
                    color:i<Math.min(student.streakCount,7)?"#fff":T.textL,
                  }}>{i+1}</div>
                ))}
              </div>
            </div>
...
function MyResultsView({ results, onBack }) {
  ...
  return (
    <div>
      ...
      <p style={{color:T.textM,fontSize:13,margin:"0 0 16px"}}>{results.length} test{results.length!==1?"s":""} completed · Tap a result to see right/wrong answers</p>
      ...
      <div style={{display:"grid",gap:12}}>
        {results.map(r=>{
          ...
          return (
            <Card key={r.id} style={{padding:0,borderLeft:`4px solid ${ringColor}`,overflow:"hidden"}}>
              <button onClick={()=>setExpandedId(isOpen?null:r.id)}
                ...
                <div style={{display:"flex",alignItems:"center",gap:14}}>
                  ...
                  <span style={{fontSize:13,color:T.textL,flexShrink:0,transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▾</span>
                </div>
              </button>
                  ...
                  <div style={{display:"grid",gap:8,marginTop:12}}>
                    {(r.answers||[]).map((a,i)=>(
                      <div key={i} style={{
                        ...
                        background:a.correct?T.success+"0d":a.selectedIndex===null?T.bg:T.error+"0d",
                        borderLeft:`3px solid ${a.correct?T.success:a.selectedIndex===null?"#94a3b8":T.error}`,
                      }}>
                        <div style={{display:"flex",justifyContent:"space-between",gap:8,marginBottom:4}}>
                          <span style={{fontSize:11,fontWeight:700,color:T.textM}}>Q{i+1}</span>
                          <span style={{fontSize:11,fontWeight:700,color:a.correct?T.success:a.selectedIndex===null?T.textL:T.error}}>
                            {a.correct?"✅ Correct":a.selectedIndex===null?"⏭ Skipped":"❌ Wrong"}
                          </span>
                        </div>
                        <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:6}}>{a.questionText}</div>
                        {!a.correct && (
                          <div style={{fontSize:12,color:T.textM}}>
                            {a.selectedIndex!==null && (
                              <div style={{color:T.error,marginBottom:2}}>Your answer: {["A","B","C","D"][a.selectedIndex]}. {(a.options||[])[a.selectedIndex]}</div>
                            )}
                            <div style={{color:T.success}}>Correct answer: {["A","B","C","D"][a.correctAnswer]}. {(a.options||[])[a.correctAnswer]}</div>
                          </div>
                        )}
                      </div>
                    ))}
...
function ProfileView({ student, myResults, onLogout, onBack }) {
  ...
  const achievements = [
    ...
    { icon:"🔥", color:"#f97316", bg:"#fff3e0", title:`${student.streakCount||0} Day Streak`, sub:"Keep it up!", unlocked: (student.streakCount||0)>0 },
  ];
